...
from check_pod import (Pod, check_pod, check_pod_not_running,
                       check_pods_num_for_this_edge_cluster,
                       check_pods_of_the_type_on_this_node,
                       check_running_pod_for_all_name, get_resource_dataframe)
...
PORT = ['5001', '5002', '5003', '5004', '5005', '5006', '5007', '5008', '5009', '5010', '5011', '5012', '5013', '5014',
        '5015', '5016', '5017', '5018', '5019', '5020']
off_load_algorithm_name = ['random-node-offload', 'random-pod-offload', 'dqn-node-offload']
...
def return_tasks_execute_situation(execute_dict: dict):
    ...
    for type in execute_dict:
        ...
        success_execute_situation[int(type) - 1] = value['success'] / (value['success'] + value['failure'])
        failure_execute_situation[int(type) - 1] = value['failure'] / (value['success'] + value['failure'])
...
def update_delete(num: int, epoch_index: int, not_running_pod_info: list, running_pod_info: list, all_pod_info: list,
                  return_result: dict):
...
# 为了pylance提供的静态类型检查
#
def local_run(index: int, current_pod: Pod, req: list,
              update_index, update_index_lock,
              arrive_from_center_to_edge_time: float, trans_from_center_to_edge: float,
              algorithm_type: int, local_offload_epoch_returned_by_func: int, pod_name_index, pod_name_index_lock):
    try:
        ...
        if current_pod is None:
            ...
            detail_mission = {'name': flow['from'],
                              'type': service_type, 'success': 0, 'failure': 1}
            ...
            alg_ip = alg_pod.ip
            pod_index = requests.post('http://' + alg_ip + ':' + alg_pod + '/predict', data=observation)
            ...
            ###########################要改时间
            runtime = end - arrive_from_center_to_edge_time + \
                      trans_from_center_to_edge * 2  # 其实这个时间是发送时间，应该最终答案传给datacenter

            print(f'task #{index}, type {service_type} ends:', flow, flush=True)

            mission = {'success': 0, 'failure': 0, 'stuck': 0}
            detail_mission = {'name': flow['from'],
                              'type': service_type, 'success': 0, 'failure': 0}
        ...
        arguments = [local_offload_epoch_returned_by_func, service_type, if_success, runtime]
        offload_algorithm_pod = check_pod(off_load_algorithm_name[algorithm_type], True, True, 2, use_cache=False)[0]
        run(f"curl http://{offload_algorithm_pod.ip}:{off_load_algorithm_port[algorithm_type]}/feedback -X POST -d 'observation={json.dumps(arguments)}'",
            None)
...
# 编排服务
def update_control(current_resource_of_nodes, return_result, current_update, pod_name_index, pod_name_index_lock):
    print('··········································update开始了')
    running_pod_info = []
    update_pod_info = []
    ...
    for service_type in range(1, SERVICE_TYPE_NUM + 1):
        temp = []
        pods_list = check_pod('service' + str(service_type), True, True, 0, use_cache=True)
        ...
        temp = []
        pods_list = check_pod_not_running('service' + str(service_type), True, 0, use_cache=True)
        for pod in pods_list:
            temp.append(pod.__dict__)
        not_running_pod_info.append(temp)

        temp = []
        pods_list = check_pod('service' + str(service_type), False, True, 0, use_cache=True)
        for pod in pods_list:
            temp.append(pod.__dict__)
    ...
    for type, num in changes.items():
        ...
        if num > 0:
            ...
            if len(pods_list) < num:
                ...
                num = len(pods_list)
            update_delete(num, epoch_index, not_running_pod_info[type - 1], running_pod_info[type - 1],
                          all_pod_info[type - 1], return_result)
...
def receive_result_from_cloud(update_index, update_index_lock, offload_algorithm_type, pod_name_index,
                              pod_name_index_lock):
    def run_once(command):
        return os.popen(command).read()
    ...
    print("·················监听云执行情况端口开启")
    receive_result_from_cloud_server = socket.socket(
        socket.AF_INET, socket.SOCK_STREAM)
    receive_result_from_cloud_server.setblocking(True)
    receive_result_from_cloud_server.setsockopt(
        socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    receive_result_from_cloud_server.bind(
        ('0.0.0.0', EDGE_MASTER_RECEIVE_RESULT_PORT))
    ...
    while True:
        ...
        check_pod(off_load_algorithm_name[offload_algorithm_type], True, True, 2, use_cache=False)[0]

        # 这里应该为offload加一个返回情况！！！！！！！！！！！
        run(f"curl http://{offload_algorithm_pod.ip}:{off_load_algorithm_port[offload_algorithm_type]}/feedback -X POST -d 'observation={json.dumps(arguments)}'",
            None)
...
def update_service(return_result, pod_name_index, pod_name_index_lock):
    ...
    update_control(current_resource_of_nodes, return_result, UPDATE, pod_name_index, pod_name_index_lock)


def offload_service(algorithm_type, req_type, offload_epoch_index, offload_epoch_index_lock):
    def run_once(command):
        return os.popen(command).read()
    ...
    for node_name in nodes_name:
        ...
        pod_ip = check_pod(resource_deploy_name, True, True, 1, use_cache=True)[0].ip  # 由主进程调用，此时主进程已经刷新了缓存
        # 是否需要-d?
        # 定的是7000端口返回资源，get请求
    ...
    offload_algorithm_pod = check_pod(off_load_algorithm_name[algorithm_type], True, True, 2, use_cache=True)[0]
    ...
    with offload_epoch_index_lock:  # 这个锁有必要吗？
        ...
        if offload_epoch_index.value == 1:
            # 加上上一个成功与否（1/0)和执行时间!!!!!!!!!!!!!!!!!!!
            arguments = [offload_epoch_index.value, 0, req_type + 1, req_type, nodes_resource]
        else:
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        p1 = multiprocessing.Process(target=receive_result_from_cloud,
                                     args=(update_index, update_index_lock, offload_algorithm_index, pod_name_index,
                                           pod_name_index_lock))
        p1.start()

        print("·················receive主进程开始")
        edge_master_receive_server = socket.socket(
            socket.AF_INET, socket.SOCK_STREAM)
        edge_master_receive_server.setblocking(True)
        edge_master_receive_server.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        edge_master_receive_server.bind(
            ('0.0.0.0', EDGE_MASTER_RECEIVE_REQUEST_PORT))
        ...
        with Pool(processes=10) as pool:
            while True:
                ...
                pod_lists = [check_pod('service' + str(i), True, True, 0, use_cache=True) for i in
                             range(1, SERVICE_TYPE_NUM + 1)]
                ...
                if len(pod_list) == 0:
                    ...
                    detail_mission = {
                        'name': master_name, 'type': request[0]}
                    ...
                    if offload_algorithm_index != 1:
                        # 注意是否只传这两个参数就可以？？？？？？？？？？？？？？？？？
                        # 是否这样返回可以？？？？？？？？？？？？？？？？？
                        node_index, offload_epoch_returned_by_func = offload_service(offload_algorithm_index,
                                                                                     req[0], offload_epoch_index,
                                                                                     offload_epoch_index_lock)
                        if node_index == 0:
                            print('发给云')
                            this_client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                            this_client.connect((CLOUD_IP, CLOUD_MASTER_RECEIVE_REQUEST_PORT))
                            # 增加edge名字
                            ...
                            offload_pod_list = check_pods_of_the_type_on_this_node('service' + str(req[0]),
                                                                                   chosen_node_name, True, 0,
                                                                                   use_cache=True)
                            ...
                            # 随机选一个
                                ...
                                selected_pod = offload_pod_list[random.randint(0, len(offload_pod_list) - 1)]
                            flow = pool.apply_async(local_run,
                                                    (local_task_count, selected_pod, req, update_index,
                                                     update_index_lock, arrive_from_center_to_edge_time,
                                                     trans_from_center_to_edge, offload_algorithm_index,
                                                     offload_epoch_returned_by_func, pod_name_index,
                                                     pod_name_index_lock))
                            local_task_count += 1
