...
def get_resource_dataframe(
    resource: Union[Literal["node"], Literal["pod"]], use_cache=False
) -> pd.DataFrame:
    if use_cache and resource in cache:
        return cache[resource]
    output = subprocess.getoutput(f"kubectl get {resource} -o wide")
    resource_dataframe = pd.read_csv(StringIO(output), sep=r"\s{2,}", engine="python")
...
class CheckPodRules(list):
    ...
    regex_list = [
        re.compile(r"^(service\d+-deployment\d*)-[0-9a-f]+-[0-9a-z]+$"),
    ...
    def __init__(
        ...
        if if_check_regex is not None:
            self.append(
                CheckPodRules.rename_by_regex(CheckPodRules.regex_list[regex_index])
            )
        self += rules
...
def check_pod_impl(
    rules: List[Callable[[Pod, Series], Optional[Pod]]] = [], use_cache=False
) -> List[Pod]:
    pod_list = []
...
def check_all_pod(if_check_regex, regex_index, use_cache=False):
    return check_pod_impl(
        CheckPodRules(
            if_check_regex=if_check_regex, regex_index=regex_index, check_running=True
        ),
        use_cache,
    )
...
def check_pod(target, require_running, if_check_regex, regex_index, use_cache=False):
    return check_pod_impl(
        CheckPodRules(
            name_starts_with=target + "-deployment",
            check_running=True if require_running else None,
            if_check_regex=if_check_regex,
            regex_index=regex_index,
        ),
        use_cache,
...
def check_running_pod_for_all_name(if_check_regex, regex_index, use_cache=False):
    return [
        pod.name
        for pod in check_all_pod(
            if_check_regex=if_check_regex, regex_index=regex_index, use_cache=use_cache
        )
    ]


def check_pod_for_its_node(
    deployment_name, service_name, if_check_regex, regex_index, use_cache=False
):
    pod_list = check_pod_impl(
        CheckPodRules(
            name_starts_with=service_name + "-deployment",
            if_check_regex=if_check_regex,
            regex_index=regex_index,
        ),
        use_cache,
    ...
    for pod in pod_list:
        if pod.name == deployment_name:
            return pod.node
    raise Exception(
        f"Pod not found with deployment_name={deployment_name} and service_name={service_name}"
    )


def check_pods_of_the_type_on_this_node(
    service_name, node_name, if_check_regex, regex_index, use_cache=False
):
...
def check_pods_num_for_this_edge_cluster(if_check_regex, regex_index, use_cache=False):
    ...
    all_pods = check_pod_impl(
        CheckPodRules(
            name_starts_with="service",
            check_running=True,
            if_check_regex=if_check_regex,
            regex_index=regex_index,
        ),
